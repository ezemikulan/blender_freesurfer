...
12. Select the edited object in the *Outliner* panel (if it is not already selected). Go to *File > Export > Wavefront (.obj)*. Navigate to the *conv* folder. Add *_edit* to the .obj file's original name (e.g. *subj_name_inner_skull_surface.obj -> subj_name_inner_skull_surface_edit.obj*). Set the options as follows and export :
